...
export default function Header({ whatsappNumber }: HeaderProps) {
  ...
  return (
    <>
      {/* Utility Bar */}
      <div className="bg-brand-black border-b border-white/10 text-xs">
      ...
      {/* Main Nav */}
      <header className="sticky top-0 z-40 bg-brand-black/98 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 flex items-center justify-between h-16 sm:h-[4.5rem]">
          ...
          <Link to="/" className="absolute left-1/2 -translate-x-1/2 flex items-center">
            <img
              ...
              alt="CasseoHair"
              className="h-11 w-auto sm:h-12 md:h-14 max-w-[min(52vw,220px)] object-contain"
            />
          </Link>
        ...
        {searchOpen && (
          <div className="px-4 pb-3 animate-fade-in border-t border-white/10">
            <form onSubmit={handleSearch}>
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-white/20 bg-white/10 text-white placeholder:text-white/50 text-sm outline-none focus:border-brand-pink focus:ring-2 focus:ring-brand-pink/30"
                autoFocus
              />
            </form>
          </div>
